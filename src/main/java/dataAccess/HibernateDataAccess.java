...
public class HibernateDataAccess {
	...
	public HibernateDataAccess() {
		...
		if (db != null && db.isOpen()) {
	        System.out.println("EntityManager opened successfully.");
	    } else {
	        System.err.println("Failed to open EntityManager!");
	    }
	...
	public void initializeDB() {
		...
		if (!db.createQuery("SELECT d FROM Driver d", Driver.class).getResultList().isEmpty()) {
	...
	public List<Ride> getRidesByPlaces(int nPlaces) {
		System.out.println(">> DataAccess: getRides=> places= " + nPlaces);

		List<Ride> res = new ArrayList<Ride>();
		TypedQuery<Ride> query = db.createQuery("SELECT r FROM Ride r WHERE r.nPlaces=?1",
				Ride.class);
		query.setParameter(1, nPlaces);
		List<Ride> rides = query.getResultList();
		for (Ride ride : rides) {
			res.add(ride);
		}
	...
	public void open() {
		try {
			emf = Persistence.createEntityManagerFactory("ridesHibernate");
			db = emf.createEntityManager();
			System.out.println("DataAccess opened successfully for persistence unit 'ridesHibernate'");
		} catch (Exception e) {
			System.err.println("Error opening DataAccess: " + e.getMessage());
